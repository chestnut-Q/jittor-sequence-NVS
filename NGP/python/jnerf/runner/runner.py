...
class Runner():
    def __init__(self):
        ...
        self.best_pkl = os.path.join(self.save_path, "best.pkl")
            ...
            if i>0 and i%self.val_freq==0:
                ...
                if psnr > best_psnr:
                    ...
                    if best_count >= 10:
                        break
                # self.test(i)
        self.render_test(True, None, True)

    def test(self, iter):
        if self.dataset["test"]  is None:
            self.dataset["test"]    = build_from_cfg(self.cfg.dataset.test, DATASETS)
        path = os.path.join(self.save_path, "test_{}".format(iter))
        if not os.path.exists(path):
            os.makedirs(path)
        mse_list=self.render_test(save_path=path)
    ...
    def valid(self, iter: int):
        if not os.path.exists(os.path.join(self.save_path, "valid_{}".format(iter))):
            os.makedirs(os.path.join(self.save_path, "valid_{}".format(iter)))
        path = os.path.join(self.save_path, "valid_{}".format(iter))
        mse_list = []
        print("rendering validset...")
        for img_i in tqdm(range(0,self.dataset["val"].n_images,1)):
            with jt.no_grad():
                imgs=[]
                for i in range(1):
                    simg, img_tar = self.render_img(dataset_mode="val", img_id=img_i)
                    imgs.append(simg)
                img = np.stack(imgs, axis=0).mean(0)
                self.save_img(path+f"/{self.exp_name}_r_{img_i}.png", img)
                self.save_img(path+f"/{self.exp_name}_gt_{img_i}.png", img_tar)
    ...
    def render_test(self, save_img=True, save_path=None, load_model=False):
        if self.dataset["test"]  is None:
            self.dataset["test"]    = build_from_cfg(self.cfg.dataset.test, DATASETS)
        if load_model == True:
            print(self.best_pkl)
            self.model.load(self.best_pkl)
        if save_path is None:
            save_path = os.path.join(self.save_path, "render")
