...
class GridEncode(Function):
    ...
    def execute(self, x,m_grid):
        ...
        # output = jt.empty([self.num_elements,32], self.grad_type) #NOTE:change
        output = jt.empty([self.num_elements,self.cfg.n_features_per_level * self.cfg.n_levels], self.grad_type)
