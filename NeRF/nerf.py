...
def config_parser():
    ...
    parser.add_argument("--log", type=str,
                    default="/work/jrender/logs/log_output/log.txt", help='redirect output log') 
...
def train(args):
    ...
    for i in range(start, N_iters):
        ...
        if i%args.i_print==0:
            time_now = time.time()
            print(f"[TRAIN] Iter: {i} Loss: {loss.item()}  PSNR: {psnr.item()}, Total time: {(time_now-start_time)/60} mins")
            if i%args.i_img==0:
                ...
                if not jt.mpi or jt.mpi.local_rank()==0:
                    ...
                    print(('test/psnr', psnr.item(), global_step))       
